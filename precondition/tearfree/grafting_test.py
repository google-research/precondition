...
def _minustwo() -> optimizers.ShardedGradientTransformation:
...
class GraftingTest(parameterized.TestCase):
  """Basic test for grafting optimizers implementations."""
  ...
  def test_no_graft(self, i):
    """Check that no graft behaves exactly as the base transform."""
    options = grafting.Options(
        grafting.GraftingType.NONE, 0.0, start_preconditioning_step=i
    )
