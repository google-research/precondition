...
class ShampooTest(parameterized.TestCase):
  ...
  def test_basic(self, ndim):
    ...
    def root(x, p):
      ...
      return v.dot(np.diag(np.maximum(0, w) ** (-1 / (2 * p)))).dot(v.T)
  ...
  def test_basic_ema(self, decay, last):
    ...
    if last:
      seq = jnp.stack([z, z, g])
      one = jnp.stack([g * (1 - decay)])
    else:
      seq = jnp.stack([g, z, z])
      one = jnp.stack([g * decay**2 * (1 - decay)])
    ...
    np.testing.assert_allclose(last, last_no_decay)
  ...
  def test_blocks_equality(self, shape, block_size):
    ...
    grads_for_each_block = [[]] * meta.num_blocks
    for grad in grads:
    ...
    for block in grads_for_each_block:
      ...
      last_grad.append(self._unroll_concrete(options, block[0], block)[1][-1])

    expected = jnp.stack(last_grad, axis=meta.blocks_axis)
    expected = shampoo._deblockify(expected, meta)
    actual = self._unroll_concrete(options, grads[0], grads)[1][-1]
    np.testing.assert_allclose(np.squeeze(expected), np.squeeze(actual))
