...
def _pspec(
    options: Options, params: pytypes.NestedHParams
) -> pytypes.NestedHParams:
  """Generate sharding specification for shampoo state."""
  count_pspec = base_layer.WeightHParams(
      shape=[],
      init=None,
      dtype=jnp.int32,
      collections=None,
  ...
  def make_blocks_pspec(
      path: ...,
      param: base_layer.WeightHParams,
  ) -> pytypes.NestedHParams:
    meta = _blocks_metadata(options, param.shape, str(path))
...
def _update_block_stats(
  ...
  with jax.named_scope("ShampooStats"):
    for axis, cov in enumerate(block.stats):
      if axis >= meta.blocks_axis:
        axis += 1

      all_axes = list(range(update.ndim))
      all_axes.remove(axis)

      new_cov = jnp.tensordot(update, update, axes=(all_axes, all_axes))
...
def _update_block_precond(
  ...
  with jax.named_scope("PthInvRoot"):
    new_roots = list(map(_pth_inv_root, block.stats))
