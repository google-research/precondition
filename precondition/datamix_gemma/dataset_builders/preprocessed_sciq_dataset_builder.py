...
"""Dataset builder for the Preprocessed SciQ dataset."""
...
sciq_train_path = '/home/shivguptashi/sciq/sciq_train.tfrecord'
...
class PreprocessedSciQDatasetBuilder(dataset_builder.DatasetBuilder):
  ...
  def __init__(
    ...
    self._train_data = tf.data.TFRecordDataset(
        [sciq_train_path], num_parallel_reads=tf.data.AUTOTUNE
    )
    self._validation_data = tf.data.TFRecordDataset(
        [sciq_validation_path], num_parallel_reads=tf.data.AUTOTUNE
    )
