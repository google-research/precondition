...
"""Dataset builder for the Wikipedia dataset."""
...
wiki_path_prefix = '/home/shivguptashi/open_orca/wiki_tokenized'
...
class PreprocessedWikipediaDatasetBuilder(dataset_builder.DatasetBuilder):
  """Dataset builder for the Open Orca dataset."""

  def __init__(
      self,
      tokenizer: gemma_tokenizer.GemmaTokenizer,
      max_seq_len: int,
      topic: int,
  ):
    """Constructor.
    ...
    Args:
      ...
      topic: topic to use for training.
    """
    self._tokenizer = tokenizer
    cur_wiki_path = wiki_path_prefix + '_topic_' + str(topic) + '.tfrecord'
    self._base_data = tf.data.TFRecordDataset(
        [cur_wiki_path], num_parallel_reads=tf.data.AUTOTUNE
    )
